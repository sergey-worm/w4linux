...
/*printk("__put_user_check:  0.\n");*/ \
/*printk("__put_user_check:  1 - STACK_TOP=0x%lx.\n", STACK_TOP);*/ \
/*printk("__put_user_check:  2 - x=0x%lx, addr=0x%lx, size=0x%lx.\n", (long)x, (long)addr, (long)size);*/ \
	if (0 || __access_ok(addr, size)) { \
		switch (size) { \
		case 1: \
		...
		case 2: \
			...
			break; \
		case 4: \
/*printk("__put_user_check:  3 - x=0x%lx, addr=0x%lx, size=0x%lx.\n", (long)x, (long)addr, (long)size);*/ \
			__put_user_asm(x, , addr, __pu_ret); \
/*printk("__put_user_check:  4 - x=0x%lx, addr=0x%lx, size=0x%lx.\n", (long)x, (long)addr, (long)size);*/ \
			break; \
		case 8: \
...
/*printk("__put_user_check:  5 - __access_ok(addr, size) - failed, ret -EFAULT.\n");*/ \
		__pu_ret = -EFAULT; \
	} \
/*printk("__put_user_check:  6 - __pu_ret=0x%x/%d.\n", __pu_ret, __pu_ret);*/ \
	__pu_ret; \
})
...
__asm__ __volatile__(							\
		"/* Put user asm, inline. */nop;nop;nop;nop;\n"				\
	"1:\t"	"st"#size " %1, %2\n\t"					\
		"clr	%0\n"						\
	"2:\n\n\t"							\
