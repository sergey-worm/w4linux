...
{
 //printk("%s:  ptep=0x%p, pteval=0x%lx, need_map_page=%d.\n", __func__, ptep, pteval, (pteval & 3) == 2);

	srmmu_swap((unsigned long *)ptep, pte_val(pteval));
}

#if 1 // wrm
static inline unsigned mmuacc2acc(int mmuacc)
{
	enum
	{
		// sparc MMU accesses:
	...
	switch (mmuacc)
	{
		case Mmu_acc_kro_uro:    return R|r;
		case Mmu_acc_krw_urw:    return R|W|r|w;
		case Mmu_acc_krx_urx:    return R|X|r|x;
		case Mmu_acc_krwx_urwx:  return R|W|X|r|w|x;
		case Mmu_acc_kxo_uxo:    return X|x;
		case Mmu_acc_krw_uro:    return R|W|r;
		case Mmu_acc_krx_uno:    return R|X;
		case Mmu_acc_krwx_uno:   return R|W|X;
		/*
...
{
 //printk("%s:  addr=0x%lx, ptep=0x%p, pteval=0x%lx, need_map_page=%d.\n",
 // __func__, addr, ptep, pteval, (pteval & 3) == 2);

	set_pte(ptep, pteval);

 if ((pteval & 3) == 2) // is pte ?
 {
  int rwx = mmuacc2acc((pteval >> 2) & 7);
  //printk("%s:  rwx=%d.\n", __func__, rwx);
  cxx_map((pteval << 4) & ~0xfff, addr, PAGE_SIZE, rwx);
 }
}
#else // not wrm
...
{
 //printk("%s:  ptep=0x%p, mm=0x%p, addr=0x%lx.\n", __func__, ptep, mm, addr);
	__pte_clear(ptep);
 cxx_unmap(addr, 0x1000);
}
...
#else
 #define VMALLOC_START           (KERNBASE + 0x0e600000)
 #define VMALLOC_END             (KERNBASE + 0x0fc00000)
#endif
