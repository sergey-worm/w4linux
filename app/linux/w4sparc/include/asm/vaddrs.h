 ...
 */
#define FIXADDR_TOP		(KERNBASE + 0x0cfff000UL)
#define FIXADDR_SIZE		((FIX_KMAP_END + 1) << PAGE_SHIFT)
#define FIXADDR_START		(FIXADDR_TOP - FIXADDR_SIZE)

#define __fix_to_virt(x)        (FIXADDR_TOP - ((x) << PAGE_SHIFT))

#define SUN4M_IOBASE_VADDR  (KERNBASE + 0x0d000000) /* Base for mapping pages */
#define IOBASE_VADDR        (KERNBASE + 0x0e000000)
#define IOBASE_END          (KERNBASE + 0x0e600000)

#define KADB_DEBUGGER_BEGVM (KERNBASE + 0x0fc00000) /* Where kern debugger is in virt-mem */
#define KADB_DEBUGGER_ENDVM (KERNBASE + 0x0fd00000)
#define DEBUG_FIRSTVADDR    (KERNBASE + KADB_DEBUGGER_BEGVM)
#define DEBUG_LASTVADDR     (KERNBASE + KADB_DEBUGGER_ENDVM)

#define LINUX_OPPROM_BEGVM  (KERNBASE + 0x0fd00000)
#define LINUX_OPPROM_ENDVM  (KERNBASE + 0x0ff00000)

#define DVMA_VADDR          (KERNBASE + 0x0ff00000) /* Base area of the DVMA on suns */
#define DVMA_END            (KERNBASE + 0x0ffc0000)
