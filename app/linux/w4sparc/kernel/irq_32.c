...
void arch_local_irq_restore(unsigned long old_psr)
{
	...
	// old_psr may be valid PSR value or irq_on/off flag
	int on = old_psr;
	if (old_psr != 0  &&  old_psr != 1)
		on = !(old_psr & 0xf00); // psr to irq_on
	wrm_irq_on = on;
...
void __init init_IRQ(void)
{
	// wrm
	printk("%s:  hello.\n", __func__);
	sparc_cpu_model = sparc_leon; // wrm:  WA cause leon is known for me
	// ~wrm
