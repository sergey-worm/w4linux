...
void cxx_map(unsigned long src_addr, unsigned long dst_addr, unsigned long sz, int acc);
...
//printk("%s:  5 - va=0x%lx, pa=0x%lx, srmmu_nocache_end=0x%lx.\n", __func__, vaddr, paddr, srmmu_nocache_end);
...
//printk("%s:  5.1 - pteval=0x%lx.\n", __func__, pteval);
/**/cxx_map(paddr, vaddr, PAGE_SIZE, 6<<3/*krw*/);

		vaddr += PAGE_SIZE;
		paddr += PAGE_SIZE;
	}
//printk("%s:  6.\n", __func__);
//l4_kdb("check 0xdc000378");
...
void w4linux_switch_mm(const struct mm_struct *old_mm, const struct mm_struct *mm, long ctxid);
	       ...
	       struct task_struct *tsk)
{
	...
	w4linux_switch_mm(old_mm, mm, mm->context);  // wrm add
...
void __init srmmu_paging_init(void)
{
 ...
 if (1) // wrm
 {
	...
	srmmu_nocache_init();
 //printk("%s:  3.1.1\n", __func__);
if(0)	srmmu_inherit_prom_mappings(0xfe400000, (LINUX_OPPROM_ENDVM - PAGE_SIZE));
 //printk("%s:  3.1.2\n", __func__);
	map_kernel();
 //printk("%s:  3.1.3\n", __func__);
...
//	srmmu_allocate_ptable_skeleton(DVMA_VADDR, DVMA_END);
 ...
 //printk("%s:  3.5\n", __func__);
if(0)	srmmu_allocate_ptable_skeleton(__fix_to_virt(__end_of_fixed_addresses - 1), FIXADDR_TOP);
if(0)	srmmu_allocate_ptable_skeleton(PKMAP_BASE, PKMAP_END);

 //printk("%s:  3.6\n", __func__);
	pgd = pgd_offset_k(PKMAP_BASE);
 //printk("%s:  3.6.1\n", __func__);
if(0)	pmd = pmd_offset(pgd, PKMAP_BASE);
 //printk("%s:  3.6.2\n", __func__);
if(0)	pte = pte_offset_kernel(pmd, PKMAP_BASE);
 //printk("%s:  3.6.3\n", __func__);
if(0)	pkmap_page_table = pte;
...
//	flush_tlb_all();
 ...
 //printk("%s:  7.\n", __func__);
		free_area_init_node(-1, zones_size, pfn_base, zholes_size);
 //printk("%s:  8.\n", __func__);
	}
 //printk("%s:  9.\n", __func__);
}
...
/*wrm*/
void wrm_flush_cache_all(void) { /*printk("%s:  FAKE.\n", __func__);*/ }
void wrm_flush_cache_mm(struct mm_struct* v) { /*printk("%s:  FAKE.\n", __func__);*/ }
void wrm_flush_cache_range(struct vm_area_struct* v1, unsigned long v2, unsigned long v3) { /*printk("%s:  FAKE.\n", __func__);*/ }
void wrm_flush_cache_page(struct vm_area_struct* v1, unsigned long v2) { /*printk("%s:  FAKE.\n", __func__);*/ }

void wrm_flush_tlb_all(void) { /*printk("%s:  FAKE.\n", __func__);*/ }
void wrm_flush_tlb_mm(struct mm_struct* v) { /*printk("%s:  FAKE.\n", __func__);*/ }
void wrm_flush_tlb_range(struct vm_area_struct* v1, unsigned long v2, unsigned long v3) { /*printk("%s:  FAKE.\n", __func__);*/ }
void wrm_flush_tlb_page(struct vm_area_struct* v1, unsigned long v2) { /*printk("%s:  FAKE.\n", __func__);*/ }

void wrm_flush_page_to_ram(unsigned long v) { /*printk("%s:  FAKE.\n", __func__);*/ }
void wrm_flush_sig_insns(struct mm_struct* v1, unsigned long v2) { /*printk("%s:  FAKE.\n", __func__);*/ }
void wrm_flush_page_for_dma(unsigned long v) { /*printk("%s:  FAKE.\n", __func__);*/ }

static struct sparc32_cachetlb_ops  wrm_cachetlb_ops __ro_after_init = {
	.cache_all    = wrm_flush_cache_all,
	.cache_mm     = wrm_flush_cache_mm,
	.cache_page   = wrm_flush_cache_page,
	.cache_range  = wrm_flush_cache_range,
	.tlb_all      = wrm_flush_tlb_all,
	.tlb_mm       = wrm_flush_tlb_mm,
	.tlb_page     = wrm_flush_tlb_page,
	.tlb_range    = wrm_flush_tlb_range,
	.page_to_ram  = wrm_flush_page_to_ram,
	.sig_insns    = wrm_flush_sig_insns,
	.page_for_dma = wrm_flush_page_for_dma,
};
/*~*/
...
/*wrm*/
 ...
 sparc32_cachetlb_ops = (const struct sparc32_cachetlb_ops*) &wrm_cachetlb_ops;
